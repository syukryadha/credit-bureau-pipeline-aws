...
from utils.pipeline_run import PipelineRun
...
def run_pipeline(INBOUND_BUCKET, key, file_bureau_response):
    with PipelineRun("Credit Risk Scoring Pipeline") as run:
        ...
        with open("final_output.txt", "w") as f:
            ...
            for row in processed_rows:
                f.write(f"{row.customer_id}|{row.credit_score}|{row.risk_tier}|{row.loan_amount}|{row.processed_date}\n")
